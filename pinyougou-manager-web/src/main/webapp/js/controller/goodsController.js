...
//将单选指定的attributeName与attributeValue的数据进行与item表中的SKU规格进行添加数据
					$scope.createItemList=function(){
		//定义这个方法每次执行此方法itemList都会被初始化参数
						$scope.entity.itemList=[{spec:{},price:0,num:9999,status:'0',isDefault:'0'}]
						//将单选的数据赋值给items变量此时它相当于是一个单选数组属性
						var items=$scope.entity.desc.specificationItems;
						//遍历这个数组
						for(var i = 0 ;i<items.length;i++){
							                                                       //将attributeName当作参数                   //注意这个attributevalue是一个数组                      
					...
					addColumn=function(list,columnName,conlumnValues){
						...
						for(var i=0;i<list.length;i++){
							//遍历$scope.entity.itemList=[{spec:{},price:0,num:9999,status:'0',isDefault:'0'}]
							var oldRow=list[i]
