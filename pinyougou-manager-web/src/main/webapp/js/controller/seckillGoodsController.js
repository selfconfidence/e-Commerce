...
app.controller('seckillGoodsController' ,function($scope,$controller   ,seckillGoodsService){	
    ...
    $scope.pushId=function($event,id){
    	if($event.target.checked){
    		$scope.seckill.push(id);
    	}else{
    	var index=$scope.seckill.indexOf(id);
    	$scope.seckill.splice(index,1);
    	}
    }
    
    $scope.checkSeckillStatus=function(status){
		seckillGoodsService.checkStatus($scope.seckill,status).success(
		   function(response){
			   if(response.flag){
				   $scope.searchs();
			   }else{
