...
public class PayController {
	...
	public Map weixinNativePay(){
		try{
			...
			TbPayLog payLog = wexinPayService.getOrderUser(SecurityContextHolder.getContext().getAuthentication().getName());
		...
		try{
			...
			while(true){ //无限循环的方式来监视
				...
				if(findPayDetails.get("trade_state").equals("SUCCESS")){
					                           //如果状态码对应上之后那就说明支付成功了
					//应传递的参数,, 流水号,以及日志ID
					String payLongId = orderService.updateUserPay(goodsId,findPayDetails.get("transaction_id"));
					//支付成功之后应该修改数据库的内容
					return new CRUDResult(true,payLongId);
