...
public class AddressController {
	 ...
	 */
	@RequestMapping("/add")
	public CRUDResult add(@RequestBody TbAddress address){
		try {
	...
	public CRUDResult update(@RequestBody TbAddress address){
		try {
	...
	return	addressService.findListUserIdAddress(userName);
	}
	
	@RequestMapping("/isAddressDefault")
	public CRUDResult isAddressDefault(Long addressId){
		try{
			addressService.isAddressDefault(addressId);
			return new CRUDResult(true,"OK!");
		}catch(Exception e){
			e.printStackTrace();
			return new CRUDResult(false, "网络延迟,请稍后重试!");
		}
	}
}
