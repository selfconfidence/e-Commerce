...
app.service('userService',function($http){
	...
	this.initOrder=function(pageStart,pageSize){
		return $http.get('../user/findOrder.do?pageStart='+pageStart+"&pageSize="+pageSize);
	}
	this.skipPay=function(orderId){
		return $http.get('../user/skipPay.do?orderId='+orderId);
	}
	this.findStatus=function(status){
		
		return $http.get('../user/findStatus.do?status='+status);
	}
	this.updateUser=function(userEntity){
		return $http.post("../user/updateUser.do",userEntity)
	}
});
